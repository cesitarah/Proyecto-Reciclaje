...
import type { getVendedorPanel } from "@/lib/data/vendedor";

type VendedorPanelProps = {
  materialInicial: string;
  initial: Awaited<ReturnType<typeof getVendedorPanel>>;
};
...
export default function VendedorPanel({
  ...
  const [ofertas, setOfertas] = useState(initial.ofertas);
  const [solicitudes, setSolicitudes] = useState(initial.solicitudes);
  const [precio, setPrecio] = useState(initial.precio);
  const materialAnterior = useRef(materialInicial);
  ...
  ========================================== */
  useEffect(() => {
    if (materialSeleccionado === materialAnterior.current) {
      return;
    }

    materialAnterior.current = materialSeleccionado;
    cargarPrecio();
    cargarOfertas();
    cargarSolicitudes();
  }, [materialSeleccionado]);
