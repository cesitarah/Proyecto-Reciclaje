...
type AdminPanelProps = {
  initial: Awaited<ReturnType<typeof import("@/lib/data/admin").getAdminDashboard>>;
};

export default function AdminPanel({ initial }: AdminPanelProps) {
  const router = useRouter();

  const [stats, setStats] = useState<Stats>(initial.stats);
  const [materiales, setMateriales] = useState<Material[]>(initial.materiales);
            ...
            <div className="rounded-xl border border-[#A8D5BA] bg-[#DDEFE1] p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:bg-[#D2EBDD] hover:shadow-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-2 text-2xl font-bold text-[#26382C]">
                    {stats.materialReciclado.toLocaleString("es-BO")}
